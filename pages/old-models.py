...
def update_totals(sender, **kwargs):
    print('Updating Pizza Price')
    kwargs['instance'].update_price()


def update_totals_order(sender, **kwargs):
    print('Updating Order Price')
    kwargs['instance'].order.update_price()


m2m_changed.connect(update_totals, sender=Pizza.toppings.through)
m2m_changed.connect(update_totals_order, sender=Pizza.toppings.through)
